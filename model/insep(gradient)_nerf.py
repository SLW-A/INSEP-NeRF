...
class ODEFun(torch.nn.Module):
    def __init__(self, model, target_poses, lambda_t=0.0425, beta=8):
        ...
        self.prev_grad_R = None
        self.prev_direction_R = None
        self.iteration = 0

    def forward(self, t, T_flat):
        T = T_flat.view(-1, 3, 4)
        R = T[:, :3, :3]
        t_vec = T[:, :, 3]
        ...
        return -grad.view(-1)  # Return flattened gradient for the optimizer to use
    ...
    def calculate_gradient_R(self, R, R_target):
        ...
        grad_V1 = grad_V @ term1

        grad_V2 = self.beta * R @ (RTR - I3)

        # Symmetrization operator
        sym_part = torch.matmul(RT, grad_V) + torch.matmul(grad_V.transpose(1, 2), R)
        grad_V3 = 0.5 * R @ sym_part

        # Total gradient
        grad_R = grad_V1 + grad_V2 - grad_V3
...
class Model(nerf.Model):
    ...
    def train_iteration(self, opt, var, loader):
        ...
        initial_T = self.graph.optimised_training_poses.weight.data.view(-1).requires_grad_(True)
        t = torch.linspace(0, 1, 10, device=opt.device)
        T_solved = odeint(ode_fun, initial_T, t, method='euler')[-1]
