...
class ODEFun(torch.nn.Module):
    def __init__(self, model, target_poses,  gamma=0.01, lambda_t=0.0425, beta=8):
        super(ODEFun, self).__init__()
        self.model = model
        self.gamma = gamma
        self.beta = beta
        self.lambda_t = lambda_t
        self.target_poses = target_poses.view(-1, 3, 4).to(model.graph.optimised_training_poses.weight.device)
        self.identity = torch.eye(3, device=self.target_poses.device)


    def forward(self, t, state):
        num_elements = state.shape[0] // 2
        T_flat = state[:num_elements]
        Z_flat = state[num_elements:]

        T = T_flat.view(-1, 3, 4)
        Z = Z_flat.view(-1, 3, 4)
        R = T[:, :3, :3]
        t_vec = T[:, :, 3]
        R_target = self.target_poses[:, :3, :3]
        t_target = self.target_poses[:, :, 3]
        grad_R = self.calculate_gradient_R(R, R_target)
        grad_t = 2 * (1 + self.lambda_t) * (t_vec - t_target)

        # Compute gradient for entire T matrix
        grad = torch.zeros_like(T)
        grad[:, :3, :3] = grad_R  # Set gradient of R
        grad[:, :, 3] = grad_t  # Set gradient of t
        Z_prime = -self.gamma * Z - grad

        return -torch.cat([Z.view(-1), Z_prime.view(-1)], dim=0)

    def calculate_gradient_R(self, R, R_target):
        I3 = self.identity
        RT = R.transpose(1, 2)
        RTR = torch.matmul(RT, R)
        term1 = (3 / 2) * I3 - (1 / 2) * RTR
        diff_RTR = 2 * R  # Assuming d(R^T R)/dR = 2R, simplifying the actual derivation
        G_R = R @ term1 - R_target
        term2 = term1 - 1 / 2 * R @ diff_RTR
        G_x = 2 * G_R @ term2

        grad_V1 = G_x @ term1

        grad_V2 = self.beta * R @ (RTR - I3)

        # Symmetrization operator
        sym_part = torch.matmul(RT, G_x) + torch.matmul(G_x.transpose(1, 2), R)
        grad_V3 = 0.5 * R @ sym_part

        # Total gradient
        grad_R = grad_V1 - grad_V3 + grad_V2

        return grad_R

    def calculate_objective(self, R, R_target):
        norm_diff = torch.norm(R @ (1.5 * self.identity - 0.5 * R.transpose(1, 2) @ R) - R_target, p='fro', dim=(1, 2))
        ortho_loss = self.beta * torch.norm(R.transpose(1, 2) @ R - self.identity, p='fro', dim=(1, 2))**2
        return norm_diff**2 + ortho_loss
...
class Model(nerf.Model):
    ...
    def build_networks(self,opt):
        ...
        if opt.camera.noise:
            weights = torch.tensor([0.4, 0.6], device=opt.device)
            means = [torch.tensor([0.0, 0.0, 0.0], device=opt.device),
                     torch.tensor([0.0, 0.0, 0.0], device=opt.device)]
            stds = [torch.tensor([opt.camera.noise_r, opt.camera.noise_r, opt.camera.noise_r], device=opt.device),
                    torch.tensor([0.5 * opt.camera.noise_r, 0.5 * opt.camera.noise_r, 0.5 * opt.camera.noise_r],
                                 device=opt.device)]

            so3_noise = mixed_gaussian_noise(len(self.train_data), 3, opt.device, weights, means, stds)
            t_noise = mixed_gaussian_noise(len(self.train_data), 3, opt.device, weights, means, stds)

            self.graph.pose_noise = torch.cat([camera.lie.so3_to_SO3(so3_noise), t_noise[..., None]], dim=-1)
    ...
    def train_iteration(self,opt,var,loader):
        ...
        if opt.optim.warmup_pose:
            ...
            self.optim_pose.param_groups[0]["lr"] *= min(1, self.it**2 / opt.optim.warmup_pose**2)

        pose_GT = self.get_all_training_poses(opt)[1]
        ode_fun = ODEFun(self, pose_GT).to(opt.device)
        T0_flat = self.graph.optimised_training_poses.weight.data.view(-1).requires_grad_(True)
        Z0_flat = torch.zeros_like(T0_flat)  # 初始Z为0
        initial_state = torch.cat([T0_flat, Z0_flat], dim=0)
        num_elements = initial_state.shape[0] // 2
        t = torch.linspace(0, 1, 10, device=opt.device)
        final_state = odeint(ode_fun, initial_state, t, method='euler')[-1]
        T_solved= final_state[:num_elements]
        self.graph.optimised_training_poses.weight.data = T_solved.view(-1, 3, 4).detach()
...
def mixed_gaussian_noise(batch_size, dimension, device, weights, means, stds):
    components = torch.multinomial(weights, batch_size, replacement=True)
    noise = torch.zeros(batch_size, dimension, device=device)
    for i, (mean, std) in enumerate(zip(means, stds)):
        mask = (components == i)
        noise[mask] = torch.randn(mask.sum(), dimension, device=device) * std + mean
    return noise
